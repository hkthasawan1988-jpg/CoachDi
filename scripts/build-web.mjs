...
const names = (await readdir(root)).filter(name =>
  ...
  /^(auth-session\.js|mobile-layout\.css|mobile-layout\.js|app-experience\.(?:js|css)|schedule-core\.js|native-push\.js|athlete-refunds(?:-core)?\.(?:js|css)|payout-approval(?:-core)?\.(?:js|css)|_headers)$/.test(name));
