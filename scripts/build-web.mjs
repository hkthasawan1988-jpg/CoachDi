...
const names = (await readdir(root)).filter(name =>
  ...
  /^(account-session\.js|account-views\.(?:js|css)|admin-live-ui\.js|app-check-bridge\.js|auth-session\.js|booking-server-client(?:-core)?\.js|brand-logo-ui\.js|chat-server-client\.js|coach-appointments-ui\.js|coach-approval-(?:hardening|inbox|recovery|safety|transaction)-ui\.js|coach-rejection-ui\.js|(?:coach-operations-ui|group-class-(?:server-client|ui))\.js|legacy-sprint-(?:3\.9\.[568]|4\.[012])-ui\.js|mobile-layout\.css|mobile-layout\.js|app-experience\.(?:js|css)|schedule-core\.js|native-push\.js|athlete-refunds(?:-core)?\.(?:js|css)|payout-approval(?:-core)?\.(?:js|css)|app-updates(?:-core)?\.(?:js|css)|_headers)$/.test(name));
