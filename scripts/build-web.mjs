...
const names = (await readdir(root)).filter(name =>
  ...
  /^coach-di-[\w-]+\.(png|jpg)$/.test(name) || /^(athlete-navigation\.(?:js|css)|court-theme(?:-core)?\.(?:js|css))$/.test(name) ||
  /^(auth-session\.js|mobile-layout\.css|mobile-layout\.js|app-experience\.(?:js|css)|schedule-core\.js|native-push\.js|athlete-refunds(?:-core)?\.(?:js|css)|payout-approval(?:-core)?\.(?:js|css)|app-updates(?:-core)?\.(?:js|css)|_headers)$/.test(name));
