...
const names = (await readdir(root)).filter(name =>
  ...
  /^(account-session\.js|account-views\.(?:js|css)|app-check-bridge\.js|auth-session\.js|booking-server-client(?:-core)?\.js|mobile-layout\.css|mobile-layout\.js|app-experience\.(?:js|css)|schedule-core\.js|native-push\.js|athlete-refunds(?:-core)?\.(?:js|css)|payout-approval(?:-core)?\.(?:js|css)|app-updates(?:-core)?\.(?:js|css)|_headers)$/.test(name));
