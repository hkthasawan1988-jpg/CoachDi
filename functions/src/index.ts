...
export const saveRefundAccount = onCall({ region: 'asia-southeast1', secrets: [REFUND_ACCOUNT_KEY] }, async request => {
  ...
  const now = Date.now();
  await db.ref(`privateRefundAccounts/${uid}`).set({ bankCode, bankName, accountName, accountLast4: accountNumber.slice(-4), accountNumberEncrypted: encrypted, updatedAt: now });
  await db.ref(`athleteRefundAccountSummary/${uid}`).set({ bankCode, bankName, accountName, maskedAccountNumber: maskedAccount(accountNumber), updatedAt: now });
  await db.ref(`serverAuditLogs/${uid}`).push({ type: 'refund_account_saved', createdAt: ServerValue.TIMESTAMP });
...
export const requestRefund = onCall({ region: 'asia-southeast1' }, async request => {
  ...
  const [bookingSnap, accountSnap] = await Promise.all([db.ref(`bookings/${bookingId}`).get(), db.ref(`privateRefundAccounts/${uid}`).get()]);
  const booking = bookingSnap.val();
  const account = accountSnap.val();
  if (!booking || booking.athleteId !== uid) throw new HttpsError('not-found', 'ไม่พบรายการจอง');
  if (!account?.accountNumberEncrypted || !account?.accountLast4) throw new HttpsError('failed-precondition', 'กรุณากรอกบัญชีสำหรับรับเงินคืนก่อน');
  if (!['confirmed', 'cancelled', 'declined', 'rejected_by_coach'].includes(String(booking.status))) throw new HttpsError('failed-precondition', 'สถานะการจองนี้ยังไม่สามารถขอคืนเงินได้');
  ...
  const now = Date.now();
  const refund = { bookingId, athleteId: uid, coachId: booking.coachId || '', amountSatang, status: 'requested', refundMethod: 'bank_transfer', bankCode: account.bankCode, bankName: account.bankName, accountName: account.accountName, maskedAccountNumber: `••••${account.accountLast4}`, requestedAt: now, createdAt: now, updatedAt: now };
  const updates: Record<string, unknown> = {};
  updates[`refunds/${refundId}`] = refund;
  updates[`privateRefundInstructions/${refundId}`] = { athleteId: uid, bookingId, bankCode: account.bankCode, bankName: account.bankName, accountName: account.accountName, accountLast4: account.accountLast4, accountNumberEncrypted: account.accountNumberEncrypted, createdAt: now };
  updates[`notifications/${uid}/refund_${bookingId}`] = { type: 'refund_requested', bookingId, recipientId: uid, senderId: 'system', title: 'รับคำขอคืนเงินแล้ว', message: `จะคืนเงินเข้าบัญชี ${account.bankName} ••••${account.accountLast4}`, read: false, createdAt: now };
  await db.ref().update(updates);
  await db.ref(`serverAuditLogs/${uid}`).push({ type: 'refund_requested', bookingId, refundId, amountSatang, createdAt: ServerValue.TIMESTAMP });
...
export const approveBooking = onCall({ region: 'asia-southeast1' }, async request => {
  ...
  if (!['pending_coach_approval', 'pending_verification', 'payment_uploaded', 'payment_submitted'].includes(String(booking.status))) throw new HttpsError('failed-precondition', 'สถานะรายการจองไม่พร้อมอนุมัติ');
  const now = Date.now();
  await ref.update({ status: 'confirmed', confirmedAt: now, confirmedBy: uid, updatedAt: now });
  await db.ref(`notifications/${booking.athleteId}`).push({ type: 'booking_confirmed', bookingId, recipientId: booking.athleteId, senderId: uid, title: 'Coach ยืนยันการจองแล้ว', message: 'การจองของคุณได้รับการยืนยันแล้ว', read: false, createdAt: now });
...
export const declineBooking = onCall({ region: 'asia-southeast1' }, async request => {
  ...
  await db.ref(`notifications/${booking.athleteId}`).push({ type: 'booking_declined', bookingId, recipientId: booking.athleteId, senderId: uid, title: 'Coach ไม่สามารถรับการจองนี้ได้', message: reason, read: false, createdAt: now });
  await db.ref(`serverAuditLogs/${uid}`).push({ type: 'booking_declined', bookingId, reason, createdAt: ServerValue.TIMESTAMP });
  return { ok: true, status: 'declined' };
});

export { getRefundAccountSummary, listMyRefunds, listRefundQueue, getRefundInstruction, setRefundProcessing, markRefundPaid } from './refund-admin.js';
export { getAdminPlatformOverview, getAdminCoachSchedule } from './platform-admin.js';
export { submitCoachPayoutAccount, getCoachPayoutAccountSummary, listCoachPayoutApprovals, reviewCoachPayoutAccount, revealCoachPayoutAccount } from './coach-bank-approval.js';
export { attachOmiseCardAndActivateSubscription, getCoachSubscriptionStatus, chargeDueCoachSubscriptions } from './billing-omise.js';
