...
function load(run){
  const calls={database:[],callable:[],initialized:0,command:[],create:[],athlete:[],refund:[],proof:[],group:[],groupProof:[],chat:[],supportChat:[]};const database={name:'db'},storage={name:'storage'};
...
test('mobile deployment retains existing triggers and adds App Check callables',()=>{
  ...
  assert.deepEqual(Object.keys(exported),['syncCoachBookingSchedule','syncCoachPayoutVerification','executeBookingCommand','createBookingCommand','executeAthleteBookingCommand','executeBookingRefundCommand','getBookingProofUrl','executeGroupClassCommand','getGroupClassProofUrl','sendBookingChatMessage','sendSupportChatMessage']);
  assert.equal(calls.initialized,1);assert.equal(calls.database.length,2);assert.equal(calls.callable.length,9);
...
test('callable requires Firebase Auth and forwards only verified uid plus request data',async()=>{
  ...
  assert.deepEqual(await exported.getBookingProofUrl({auth:{uid:'coach_12345678'},data:{bookingId:'BOOKING_123',kind:'payment'}}),{ok:true});
