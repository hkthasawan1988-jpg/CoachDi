...
async function setup(granted=true) {
  ...
  const session={configureSession:async data=>{calls.push(data.enabled?'native-on':'native-off');},getSession:async()=>({uid:''}),getPending:async()=>({}),clearPending:async()=>{}};
  ...
  return {sandbox,calls,writes,elements,events,authChanged};
}
test('native push requests permission on user action and records an owner-scoped Android token',async()=>{
  ...
  for(let i=0;i<20&&!t.writes.length;i++)await tick();
