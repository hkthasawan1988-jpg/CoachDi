(() => {
  ...
  renderSchedule=function(){
    ...
    const venues=new Map(),records=hours.map(hour=>days.map(date=>{const info=slot(date,hour);if(['coach_location','booked'].includes(info.type)&&!venues.has(info.label))venues.set(info.label,core.venueLabel(info.label));return info;}));
    ...
    table.innerHTML=`<thead><tr><th class="timeHead" scope="col">เวลา</th>${days.map((date,i)=>`<th class="dayHead ${date===TODAY?'today':''}" scope="col" title="${escape(shortThaiDate(date))}"><div class="dow">${shortDays[i]}</div><div class="dom">${Number(date.slice(-2))}</div><div class="monthLabel">${escape(new Date(date+'T12:00:00').toLocaleDateString('th-TH',{month:'short'}))}</div></th>`).join('')}</tr></thead><tbody>${hours.map((hour,row)=>`<tr><th class="timeCell" scope="row">${fmt(hour)}</th>${days.map((date,col)=>{const info=records[row][col],code=venues.get(info.label),selected=state.selectedCell?.date===date&&state.selectedCell?.h===hour,label=code||(['available'].includes(info.type)?(info.past||info.beyond?'—':'ว่าง'):info.type==='timeoff'?'หยุด':'—');return `<td><button type="button" class="daySlot ${info.type} ${selected?'selected':''} ${info.past||info.beyond?'cdPastSlot':''}" data-cd-slot="${date}" data-cd-hour="${hour}" aria-label="${escape(shortThaiDate(date)+' '+fmt(hour)+' '+info.label+(info.readOnly?' ดูรายละเอียด':' เลือกเวลา'))}" title="${escape(info.label)}"><span class="venue">${escape(label)}</span>${code?`<span class="cdSlotStatus">${info.type==='booked'?'จองแล้ว':'โค้ช'}</span>`:''}</button></td>`;}).join('')}</tr>`).join('')}</tbody>`;
